...
class Encoder(nn.Module):
    ...
    def forward(self, obs_traj):
        """ Forward function of the Trajectory Encoder.
            ...
            Args:
                obs_traj (tensor): Tensor of shape (obs_len, batch_size, 2).
...
class Decoder(nn.Module):
    """Decoder, part of Generator that predicts displacements.
        ...
        Args:
            pred_len (int): Length of the sequence to predict.
            ...
            mlp_dim (int): Central mlp layer dim after pooling. Defaults to 1024.
            num_layers (int): Number of stacked lstms. Defaults to 1.
            pool_every_timestep (bool): Whether to use pooling. Defaults to True.
            dropout (float): Dropout for LSTM and pooling. Defaults to 0.0.
            bottleneck_dim (int): Bottleneck dim for pooling. Defaults to 1024.
            activation (str): Either relu or leakyrelu. Defaults to 'relu'.
            batch_norm (bool): Whether to use batchnormalization in pooling. Defaults to True.
            pooling_type (str): Either pool_net or spool. Defaults to 'pool_net'.
            neighborhood_size (float): Neighborhood size for social pooling. Defaults to 2.0.
            grid_size (int): Gridsize for social pooling. Defaults to 8.

        Attributes:
            pred_len (int): Length of the sequence.
            embedding_dim (int): Output dim of embedding (2 -> embedding_dim, via Linear layer).
            ...
            mlp (nn.Module): MLP after pooling.
            hidden2pos (nn.Linear): Linear layer, transforming LSTMs hidden to coordinates.
    """

    def __init__(self, pred_len, embedding_dim=64, h_dim=128, mlp_dim=1024, num_layers=1, pool_every_timestep=True,
                 dropout=0.0, bottleneck_dim=1024, activation='relu', batch_norm=True, pooling_type='pool_net',
        ...
        if pool_every_timestep:
            if pooling_type == 'pool_net':
                ...
                self.pool_net = SocialPooling(h_dim=self.h_dim,
                                              activation=activation,
                                              batch_norm=batch_norm,
                                              dropout=dropout,
    ...
    def forward(self, last_pos, last_pos_rel, state_tuple, seq_start_end):
        ...
        Args:
            last_pos (tensor): Last positions of shape (batch, 2). # Important: Only last.
            last_pos_rel (tensor): Last displacements of shape (batch, 2).
            state_tuple (tensor): Hidden state of Generator (hh, ch). Each tensor of shape (num_layers, batch, h_dim).
            seq_start_end: A list of tuples which delimit sequences within batch.

        Returns:
            tensor: tensor of shape (self.pred_len, batch, 2).
        """
        batch = last_pos.size(0)
        pred_traj_fake_rel = []
        decoder_input = self.embedding(last_pos_rel)
        ...
        for _ in range(self.pred_len):
            ...
            if self.pool_every_timestep:
                ...
                state_tuple = (decoder_h, state_tuple[1])  # Important: pooling changes the state tuple.

            embedding_input = rel_pos

            decoder_input = self.embedding(embedding_input)
            decoder_input = decoder_input.view(1, batch, self.embedding_dim)
...
class TrajectoryGenerator(nn.Module):
    ...
    def __init__(self, obs_len, pred_len, embedding_dim=64, encoder_h_dim=64,
        ...
        if pooling_type == 'pool_net':
            ...
            self.pool_net = SocialPooling(h_dim=encoder_h_dim,
                                          activation=activation,
                                          batch_norm=batch_norm,
                                          dropout=dropout,
                                          neighborhood_size=neighborhood_size,
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, user_noise=None):
        ...
        Returns:
            Tensor of shape (self.pred_len, batch, 2)
        """
...
class TrajectoryDiscriminator(nn.Module):
    ...
    def __init__(self, obs_len, pred_len, embedding_dim=64, h_dim=64, mlp_dim=1024, num_layers=1,
        ...
        if d_type == 'global':
            ...
            self.pool_net = PoolHiddenNet(embedding_dim=embedding_dim,
                                          h_dim=h_dim,
